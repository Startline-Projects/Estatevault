...
interface VaultFarewellViewProps {
  onBack: () => void;
}

export default function VaultFarewellView({ onBack }: VaultFarewellViewProps) {
  ...
  async function handleUploadComplete() {
    ...
    setNewTitle("");
    setNewRecipient("");
    // Show the list spinner and load fresh data BEFORE the new tile renders,
    // so the list never flashes without the just-uploaded message.
    setLoading(true);
    setMode("list");
    await fetchMessages();
  }
  ...
  if (!loading && !isSubscribed) {
    return (
      <div className="space-y-6">
        <button onClick={onBack} className="inline-block text-sm text-navy hover:text-gold transition-colors">&larr; Back to Vault</button>
        <div>
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {messages.map((msg) => (
            <div key={msg.id} className="animate-fade-in rounded-xl bg-white border border-gray-200 p-5 flex items-center justify-between">
              <div>
              ...
              <div className="flex items-center gap-2">
                {msg.fileSizeMb != null && (
                  <button onClick={() => handlePreview(msg)} disabled={previewingId !== null} className="text-xs font-medium px-3 py-1.5 rounded-lg bg-navy/10 text-navy hover:bg-navy/20 transition-colors disabled:opacity-50">
                    {previewingId === msg.id ? "Loading…" : "Preview"}
                  </button>
                )}
                {msg.status !== "unlocked" && (
                  <button
