...
// This file is intentionally split: server data fetch wrapper at the bottom,
// full branded client page component here.
...
const faqs = [
  ...
  { q: "How long does it take?", a: "Most clients complete the quiz and intake in 10–15 minutes. Your documents are generated immediately after purchase." },
...
function AccordionItem({ q, a, isOpen, onToggle, accentColor }: { q: string; a: string; isOpen: boolean; onToggle: () => void; accentColor: string }) {
  return (
    <div className={`border rounded-xl transition-all duration-300 ${isOpen ? "bg-navy-50/50 border-navy-100" : "border-transparent hover:bg-gray-50"}`}>
      <button onClick={onToggle} className="flex w-full items-center justify-between px-6 py-5 text-left">
        <span className={`text-base font-medium transition-colors duration-200 ${isOpen ? "text-navy" : "text-charcoal/80"}`}>{q}</span>
        <span
          className={`ml-4 flex h-7 w-7 shrink-0 items-center justify-center rounded-full transition-all duration-300 text-white`}
          style={{ background: isOpen ? accentColor : "#e5e7eb", color: isOpen ? "white" : "#9ca3af", transform: isOpen ? "rotate(180deg)" : "none" }}
        >
      ...
      <div className="accordion-content" style={{ maxHeight: isOpen ? "200px" : "0px", opacity: isOpen ? 1 : 0 }}>
...
function PartnerPage({ branding }: { branding: PartnerBranding }) {
  ...
  return (
    <>
      {/* ── HEADER ─────────────────────────────────────────────────────────── */}
      <header className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? "bg-white/95 backdrop-blur-md shadow-premium" : "bg-white"} border-b`} style={{ borderColor: accentColor + "33" }}>
        <div className="mx-auto max-w-7xl flex items-center justify-between px-6 py-3">
          {/* Logo */}
          <a href="#" className="flex items-center gap-3 group">
            {logoUrl ? (
              <Image src={logoUrl} alt={companyName} width={44} height={44} className="h-10 w-auto transition-transform duration-300 group-hover:scale-105" />
            ) : (
              <span className="text-xl font-bold text-navy tracking-tight">{companyName}</span>
            )}
            {logoUrl && (
              <span className="text-sm font-semibold text-navy hidden sm:inline">{companyName}</span>
            )}
          </a>
          ...
          <nav className="hidden lg:flex items-center gap-8">
            {navLinks.map((link) => (
              <a key={link.href} href={link.href} className="relative text-sm font-medium text-charcoal/70 hover:text-navy transition-colors duration-200 after:absolute after:bottom-[-4px] after:left-0 after:h-[2px] after:w-0 after:transition-all after:duration-300 hover:after:w-full" style={{ ["--tw-after-bg" as string]: accentColor }}>
                {link.label}
              </a>
            ))}
          </nav>
          ...
          <div className="hidden lg:flex items-center gap-3">
            ...
            <Link href={quizHref} className="rounded-full px-5 py-2.5 text-sm font-semibold text-white transition-all duration-200 shadow-sm hover:opacity-90" style={{ background: accentColor }}>
              Protect Your Family
            </Link>
          </div>
        ...
        <div className={`lg:hidden overflow-hidden transition-all duration-300 ${mobileOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}>
          <div className="border-t bg-white px-6 pb-6" style={{ borderColor: accentColor + "1a" }}>
            <nav className="flex flex-col gap-1 pt-4">
              {navLinks.map((link) => (
                <a key={link.href} href={link.href} onClick={() => setMobileOpen(false)} className="text-sm font-medium text-charcoal/70 hover:text-navy rounded-lg px-3 py-2.5 transition-colors">
                  {link.label}
                </a>
              ))}
              <div className="mt-3 flex flex-col gap-2">
                <Link href="/auth/login" className="text-center rounded-lg border border-navy/20 px-4 py-2.5 text-sm font-medium text-navy transition-colors">
                  Sign In
                </Link>
                <Link href={quizHref} className="text-center rounded-full px-4 py-2.5 text-sm font-semibold text-white transition-colors" style={{ background: accentColor }}>
                  Protect Your Family
                </Link>
              </div>
            </nav>
            ...
            <div className="mx-auto max-w-4xl text-center">
              ...
              <div className="mt-10 flex flex-col items-center gap-4">
                <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
                  <Link href={willHref} className="group relative w-full sm:w-auto rounded-full bg-white/10 border border-white/25 px-10 py-4 text-base font-semibold text-white transition-all duration-300 hover:bg-white/20 hover:border-white/40 hover:scale-[1.02] active:scale-[0.98] text-center">
                    Create a Will
                  </Link>
                  <Link href={trustHref} className="group relative w-full sm:w-auto rounded-full bg-white/10 border border-white/25 px-10 py-4 text-base font-semibold text-white transition-all duration-300 hover:bg-white/20 hover:border-white/40 hover:scale-[1.02] active:scale-[0.98] text-center">
                    Create a Trust
                  </Link>
                </div>
                <Link href={quizHref} className="group relative w-full sm:w-auto rounded-full px-10 py-4 text-base font-semibold text-white transition-all duration-300 hover:opacity-90 hover:scale-[1.02] active:scale-[0.98] text-center overflow-hidden shadow-lg" style={{ background: accentColor }}>
            ...
            <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-6">
              {[
                ...
                <div key={step.number} className="relative group">
                  {idx < 2 && <div className="hidden md:block absolute top-12 left-[60%] w-[80%] h-px" style={{ background: `linear-gradient(to right, ${accentColor}66, ${accentColor}1a)` }} />}
                  <div className="relative flex flex-col items-center p-8 rounded-2xl bg-white border border-gray-100 shadow-premium hover:shadow-premium-lg hover:border-opacity-20 transition-all duration-300 hover:-translate-y-1" style={{ ["--hover-border" as string]: accentColor }}>
                    <div className="absolute -top-3 left-6 px-3 py-0.5 rounded-full text-xs font-bold" style={{ background: accentColor + "1a", color: accentColor }}>
                      Step {step.number}
                    </div>
          ...
          <div className="mx-auto max-w-4xl text-center">
            ...
            <div className="mt-16 grid grid-cols-1 md:grid-cols-2 gap-8">
              {[
                ...
                <div key={pkg.title} className="relative group rounded-2xl bg-white p-8 text-left transition-all duration-300 hover:-translate-y-1" style={{ border: pkg.popular ? `2px solid ${accentColor}` : "1px solid #e5e7eb", boxShadow: pkg.popular ? `0 4px 24px ${accentColor}22` : undefined }}>
                  {pkg.popular && (
                    <div className="absolute -top-4 left-1/2 -translate-x-1/2">
                      <span className="inline-flex items-center gap-1.5 rounded-full px-5 py-1.5 text-xs font-bold text-white shadow-lg animate-shimmer" style={{ background: accentColor }}>
                  ...
                  <ul className="mt-6 space-y-3.5">
                    {pkg.features.map((f) => (
                      <li key={f} className="flex items-start gap-3 text-sm text-charcoal/70">
                        <svg className="mt-0.5 w-5 h-5 shrink-0" style={{ color: accentColor }} fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg>
                        {f}
                      </li>
                    ))}
                  </ul>
                  <Link href={pkg.href} className="mt-8 block w-full rounded-xl py-3.5 text-center text-sm font-semibold text-white transition-all duration-200 hover:opacity-90 hover:scale-[1.01]" style={{ background: pkg.popular ? accentColor : "#1C3557" }}>
                    {pkg.cta}
                  </Link>
                </div>
              ))}
          ...
          <div className="relative z-10 mx-auto max-w-5xl text-center">
            ...
            <div className="mt-16 grid grid-cols-2 md:grid-cols-3 gap-4">
              {vaultCategories.map((cat) => (
                <div key={cat.label} className="group glass rounded-2xl p-6 text-center hover:bg-white/12 transition-all duration-300 hover:-translate-y-0.5" style={{ ["--hover-border" as string]: accentColor }}>
                  <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl mb-4 group-hover:scale-110 transition-transform duration-300" style={{ background: accentColor + "1a", color: accentColor }}>
                    {cat.icon}
                  </div>
            ...
            <Link href={quizHref} className="mt-14 inline-flex items-center gap-2 rounded-full border px-8 py-3.5 text-sm font-semibold transition-all duration-300 hover:text-white group" style={{ borderColor: accentColor + "66", color: accentColor }}>
              ...
              {testimonials.map((t) => (
                <div key={t.name} className="group rounded-2xl border border-gray-100 bg-white p-7 text-left shadow-premium hover:shadow-premium-lg transition-all duration-300 hover:-translate-y-0.5">
                  <div className="flex gap-0.5 mb-4">
                    {[...Array(5)].map((_, i) => (
                      <svg key={i} className="w-4 h-4" style={{ color: accentColor }} fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                    ))}
                  </div>
                  <p className="text-sm text-charcoal/70 leading-relaxed">&ldquo;{t.quote}&rdquo;</p>
                  <div className="mt-5 flex items-center gap-3">
                    <div className="flex h-9 w-9 items-center justify-center rounded-full text-xs font-bold text-white" style={{ background: accentColor }}>
                      {t.initials}
                    </div>
                    <div>
        ...
        <section id="faq" className="bg-gradient-to-b from-white to-gray-50 py-24 px-6">
          <div className="mx-auto max-w-3xl">
            ...
            <div className="space-y-2">
              {faqs.map((faq, index) => (
                <AccordionItem key={faq.q} q={faq.q} a={faq.a} isOpen={openFaq === index} onToggle={() => setOpenFaq(openFaq === index ? null : index)} accentColor={accentColor} />
              ))}
            </div>
          </div>
        </section>
        ...
        <section className="relative hero-gradient py-24 px-6 overflow-hidden">
          ...
          <div className="relative z-10 mx-auto max-w-3xl text-center">
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white tracking-tight leading-tight">
              ...
              <span style={{ background: `linear-gradient(135deg, ${accentColor}, ${accentColor}bb)`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>you.</span>
            </h2>
            <p className="mt-5 text-lg text-white/60">Start with a free quiz. No commitment until you&apos;re ready.</p>
            <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link href={quizHref} className="group relative rounded-full px-10 py-4 text-base font-semibold text-white transition-all duration-300 hover:opacity-90 hover:scale-[1.02] active:scale-[0.98] shadow-lg" style={{ background: accentColor }}>
      ...
      <footer className="bg-navy-900 py-16 px-6">
        <div className="mx-auto max-w-6xl">
          ...
          <p className="mt-6 text-xs text-white/40 leading-relaxed max-w-4xl">
            This platform provides document preparation services only. It does not provide legal advice. No attorney-client relationship is created by your use of this platform. Documents should be reviewed by a licensed attorney if you have questions about your specific legal situation.
          </p>
        </div>
      </footer>
    </>
...
// ─── Server Component Wrapper (data fetch) ────────────────────────────────────
...
export default async function PartnerLandingPage({ params }: { params: { "partner-slug": string } }) {
  const slug = params["partner-slug"];
  const reserved = ["pro", "auth", "dashboard", "quiz", "will", "trust", "api", "attorney-referral", "sales", "attorney", "farewell", "professionals", "contact", "privacy", "terms", "partners"];
  ...
  return <PartnerPage branding={branding} />;
}
