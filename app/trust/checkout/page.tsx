...
export default function TrustCheckoutPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      if (comp) {
        ...
        if (parsed.flagged) setAttorneyReview(true);
      }
    }
    init();
  }, [router]);
  ...
  async function handlePayment() {
    ...
    try {
      ...
      if (data.free) {
        router.push(`/trust/success?promo=true&order_id=${data.orderId}&email=${encodeURIComponent(data.email)}`);
        return;
      }
          ...
          <div className="p-6">
            ...
            <div className="flex items-center justify-between">
              ...
              {promoApplied ? (
                <span className="text-sm font-bold">
                  <span className="line-through text-gray-400 mr-2">$600</span>
            ...
            {promoApplied && (
              <div className="mt-3 rounded-lg bg-green-50 border border-green-200 px-4 py-2 text-sm text-green-700 font-medium text-center">
                Promo code applied — Trust Package is free
              </div>
            )}
          </div>
        </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-sm text-charcoal/60 leading-relaxed">
                      ...
                      ({complexity.reasons.join(", ").toLowerCase()}) that benefits from attorney review.
                    </p>
                    <p className="mt-2 text-sm text-charcoal/60">
                      A licensed Michigan attorney will review your trust before delivery. 48-hour turnaround. $300.
                    </p>
                  </div>
                </div>
                <label className="mt-4 flex items-center gap-3 cursor-pointer">
                  <input type="checkbox" checked={attorneyReview} onChange={(e) => { setAttorneyReview(e.target.checked); setDeclineAck(false); }} className="h-5 w-5 rounded border-gray-300 accent-gold" />
                  <span className="text-sm font-medium text-navy">Add Attorney Review</span>
                </label>
                {showDeclineWarning && (
                  <div className="mt-4 rounded-lg bg-amber-50 border border-amber-200 px-4 py-3">
                    <p className="text-sm text-amber-800">You&apos;re choosing to proceed without attorney review. By continuing you acknowledge your situation was flagged as complex.</p>
                    <label className="mt-3 flex items-start gap-3 cursor-pointer">
                      <input type="checkbox" checked={declineAck} onChange={(e) => setDeclineAck(e.target.checked)} className="mt-0.5 h-5 w-5 rounded border-gray-300 accent-gold" />
                      <span className="text-sm text-amber-900">I understand and wish to proceed without attorney review</span>
                    </label>
                  </div>
                )}
              </>
