...
export default function Step5Page() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (partner) {
        ...
        setDnsRecords(Array.isArray(partner.dns_records) ? partner.dns_records as DnsRecord[] : []);
        setVerified(!!partner.email_verified);
      }
    }
    load();
  }, []);
  ...
  const hasSetup = dnsRecords.length > 0;
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="space-y-5">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-charcoal/50">Must be on a domain you own.</p>
          </div>

          <div className="flex flex-wrap gap-2">
            <button onClick={handleSetup} disabled={busy} className="rounded-full bg-navy px-5 py-2 text-sm font-medium text-white hover:bg-navy/90 disabled:opacity-50">
              {hasSetup ? "Update / Regenerate Records" : "Generate DNS Records"}
            </button>
            {hasSetup && (
              <button onClick={handleVerify} disabled={busy} className="rounded-full bg-gold px-5 py-2 text-sm font-medium text-white hover:bg-gold/90 disabled:opacity-50">
                Verify
              </button>
            )}
            <button onClick={handleTest} disabled={busy || !verified} className="rounded-full border border-gray-300 px-5 py-2 text-sm font-medium text-charcoal disabled:text-gray-400 disabled:cursor-not-allowed">
              Send Test Email
            </button>
          </div>
          {msg && <p className="text-xs text-charcoal/70">{msg}</p>}
          ...
          {hasSetup && (
            <div className="mt-6">
                ...
                {dnsRecords.map((rec, i) => (
                  <div key={i} className="rounded-xl border border-gray-200 bg-white p-4">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs font-semibold text-navy uppercase tracking-wider">{rec.record || rec.type} Record</span>
                      <button
                      ...
                      >
                        {copied === `rec-${i}` ? "Copied!" : "Copy Value"}
                      </button>
                    </div>
                    <div className="grid grid-cols-3 gap-2 text-sm">
                      <div><p className="text-xs text-charcoal/50 mb-0.5">Type</p><p className="font-medium text-charcoal">{rec.type}</p></div>
                      <div><p className="text-xs text-charcoal/50 mb-0.5">Host</p><p className="font-mono text-charcoal text-xs break-all">{rec.name}</p></div>
                      <div><p className="text-xs text-charcoal/50 mb-0.5">Value</p><p className="font-mono text-charcoal text-xs break-all">{rec.value}</p></div>
                    </div>
                  </div>
                ))}
              </div>
              <div className="mt-3 flex items-center gap-2">
                {verified ? (
                  <span className="rounded-full bg-green-100 px-2.5 py-1 text-xs font-medium text-green-700">Verified</span>
                ) : (
                  <span className="rounded-full bg-amber-100 px-2.5 py-1 text-xs font-medium text-amber-700">Pending</span>
                )}
                <span className="text-xs text-charcoal/50">{verified ? "Email records verified" : "Email records not verified"}</span>
              </div>
            </div>
          )}
        </div>
