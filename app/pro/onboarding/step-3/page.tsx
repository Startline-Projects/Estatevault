...
import { getMe, updateMe } from "@/lib/api-client/partner";
...
export default function Step3Page() {
  ...
  async function handleContinue() {
    if (!accepted) return;
    // All document reviews route to EstateVault's in-house counsel. Partners do
    // not register their own reviewing attorney.
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="rounded-xl bg-white border border-gray-200 p-6">
          ...
          <div className="mt-4 space-y-2 text-sm">
            ...
            <div className="flex justify-between"><span className="text-charcoal/60">Goes to</span><span className="text-charcoal/50">EstateVault counsel</span></div>
          </div>
          <p className="mt-3 text-xs text-charcoal/60">Attorney review is handled by EstateVault&apos;s in-house counsel. The review fee goes directly to the reviewing attorney, which protects you from any fee-splitting concerns.</p>
        </div>
        <div className="rounded-xl bg-white border border-gray-200 p-6">
