...
export const POST = withRoute(async (_request: NextRequest) => {
  ...
  const { data: latestOrder } = await admin
    ...
    .limit(1)
    .single();
  if (!latestOrder) return fail("No orders found", 400);
  ...
  if (!documents || documents.length === 0) {
    return fail("Documents not ready yet", 400);
  }
  ...
  if (validLinks.length === 0) {
    return fail("Failed to generate download links", 500);
  }
  ...
  const linkHtml = validLinks
    .map(
      (l) =>
        `<p style="margin:8px 0;"><a href="${l.url}" style="color:#1C3557;font-weight:600;text-decoration:underline;">${l.name}</a></p>`,
    )
    .join("");
  ...
  return ok({ success: true, email: user.email });
});
