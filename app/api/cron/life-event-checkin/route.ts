...
export const GET = withRoute(async (req: NextRequest) => {
  ...
  const now = Date.now();
  const cutoff = new Date(now - 182 * DAY).toISOString();
  ...
  for (const [clientId, info] of Array.from(byClient.entries())) {
    ...
    if (!(await wantsNotification(admin, profileId, "life_event_reminders"))) continue;
    ...
    await clientRepo.stampLifeEventCheckin(admin, clientId);
