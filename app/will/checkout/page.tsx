...
import { checkConflict as checkConflictApi, checkoutWill, validatePromoCode } from "@/lib/api-client/checkout";
...
export default function WillCheckoutPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      const resume = findResumePoint("will", restored);
      if (resume) {
        router.push(`/will?resume=${resume.step}`);
        return;
      }
