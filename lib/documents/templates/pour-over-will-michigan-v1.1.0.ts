...
{{!--
PENDING ATTORNEY APPROVAL — the whole of Section E.

These are Mo Murshed's seven steps for the Last Will, approved 2026-09-13,
applied to the Pour-Over Will. He wrote them for one document; nobody asked for
them here. Every step is his wording unchanged — "this Will" reads correctly for
a pour-over will — except Step 5, which is the correction he approved in the
same round because the Will already contains a self-proving affidavit, as this
document does.

Two sentences the development team's own steps carried are gone with them: that
the witnesses must not be beneficiaries "under this Will or the companion
Trust", and that the original should be stored together with the Trust and the
Personal Representative told where both are. His Step 3 covers the first in
general terms. Nothing replaces the second.

Logged in PENDING_ATTORNEY_REVIEW.md as the one entry still open.
--}}
