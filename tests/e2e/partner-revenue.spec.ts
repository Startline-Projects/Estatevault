...
test.describe("partner revenue page", () => {
  test("basic partner sees revenue page without crash", async ({ page }) => {
  ...
  test("revenue page shows earnings sections (total, pending, breakdown)", async ({ page }) => {
    await loginAs(page, TEST_USERS.partnerBasic.email, TEST_USERS.partnerBasic.password);
  ...
  test("enterprise partner page also loads", async ({ page }) => {
